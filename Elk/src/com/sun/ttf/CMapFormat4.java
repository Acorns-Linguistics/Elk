...
public class CMapFormat4 extends CMap
{
  ...
  /** Creates a new instance of CMapFormat4 */
           ...
           {  if (s.hasMap)
              {  // return the index of this character in
               ...
               {  // return the character code + idDelta
            	   short delta = (short)segments.get(s);
                   int idDelta = (int)delta;
  ...
  public void setMap(char src, char glyph)
  {
     ...
     {  Segment s = i.next();
        ...
        {  // are we within range?
           ...
           {  if (s.hasMap)
              {  char[] map = (char[]) segments.get(s);
                 map[src - s.startCode] = glyph; 
                 return;
              } else
              {   
            	  short delta = (Short)segments.get(s);
            	  int idDelta = (int)delta;
                  if (glyph == src + idDelta) return;

                  short start = (short)s.startCode;
                  short end = (short)s.endCode;
                  segments.remove(s);
                  if (start!=src) addSegment(start, (short)(src-1), delta);
                  if (end!=src) addSegment((short)(src+1), end, delta);
                  addSegment( (short)src, (short)src, (short)(glyph-src) );
                  return;
              }
           } else   {  return;  } // Undefined character
        }
     }       // End for
  ...
  {  // look at each segment
     ...
     {  Segment s = i.next();
        ...
        {  char[] map = (char[]) segments.get(s);
           ...
           {  if (map[c] == glyphID) { return (char) (s.startCode + c);  }  }
        } else 
  ...
  {  // read the table size values
     ...
     // read the endCodes
     for (int i = 0; i < segCount; i++) {  endCodes[i] = data.getShort();  }
        
     // read the pad
     data.getShort();
        
     // read the start codes
     for (int i = 0; i < segCount; i++) { startCodes[i] = data.getShort(); }
        
     // read the idDeltas
     for (int i = 0; i < segCount; i++) { idDeltas[i] = data.getShort(); }
  ...
  {  ByteBuffer buf = ByteBuffer.allocate(getLength());
     ...
     {  Segment s = i.next();
        ...
        if (!s.hasMap)
        {   Short idDelta = (Short) segments.get(s);
            buf.putShort(idDelta);
     ...
     {   Segment s = i.next();
         ...
         {  // first set the offset, which is the number of bytes from the
            ...
            // now write the map
            char[] map = (char[]) segments.get(s);
            for (int c = 0; c < map.length; c++) {  buf.putChar(map[c]);  }
   ...
   */
  public short getSegmentCount() {  return (short) segments.size();  }
  ...
  {  StringBuffer buf = new StringBuffer();
     ...
     {  Segment s = i.next();
        ...
        if (!s.hasMap) { buf.append("delta: " + segments.get(s));  }
  ...
  {  /** the end code (highest code in this segment) */
    ...
    @Override public boolean equals(Object o) { return (compareTo(o) == 0);  }
        
    /** Segments sort by increasing endCode */
    public int compareTo(Object o)
    {  if (!(o instanceof Segment)) {  return -1; }
       ...
       if (endCode > s.endCode) {  return 1; }
       else if (endCode < s.endCode) {  return -1; }
       else { return 0; }
    }
  }    // End class Segmant
}      // End class CMapFormat4
